...
<header class="header fullwidth"> 
	<div class="topsec clearfix">
			...
			<form class="color-theme-form">
				<ul>
					...
					<li class="input-item zoom-wrapper"> 
						...
						<input type="button" role="button" id="A1" class="increase" onclick="textType()" value=" A+ ">
						<input type="button" role="button" id="A2" class="resetMe" onclick="textType()" value=" A ">
						<input type="button" role="button" id="A3" class="decrease" onclick="textType()" value=" A- ">
		...
		<div class="block-right clearfix">
			...
			<ul class="contact-details-top clearfix">
				...
				if(!empty($SITE_EMAIL)){
					?>
					<li><a href="mailto:{{$SITE_EMAIL}}"><i class="mailicon"></i> <span>{{$SITE_EMAIL}}</span></a></li>
					<?php
				}
				if(!empty($SITE_PHONE)){
					?>
					<li><a href="tel:{{$SITE_PHONE}}"><i class="phoneicon"></i> <span>{{$SITE_PHONE}}</span></a></li>
					<?php
				}
				?>
			</ul>
			<ul class="social-links clearfix">
		          ...
		          if(!empty($FACEBOOK)){
		            ?>
		            <li><a class="facebook-link" href="{{$FACEBOOK}}" target="_blank"><i class="facebookicon"></i></a></li>
		            <?php
		          }
		          if(!empty($YOUTUBE)){  
		            ?>
		            <li><a class="youtube-link" href="{{$YOUTUBE}}" target="_blank"><i class="youtubeicon"></i></a></li>
		            <?php
		          }
		          /*if(!empty($TWITTER)){
		            ?>
	...
	<div class="logo">
		<a href="{{url('/')}}"><img src="{{$logoUrl}}" alt="Blind Welfare Society" border="0" /></a>
	</div>
	<?php
			...
			*/?>
			<ul>
				...
				<li>
					...
					<ul class="sub-menu">
						<li><a href="{{url('indian')}}">Indian</a></li>
						<li><a href="{{url('international')}}">International</a></li>
						<li><a href="{{url('donate-online')}}">Donate Online</a></li>
						<li><a href="{{url('donate-for-a-kind')}}">Donate for a Kind</a></li>
					</ul>
				</li>
				<li><a href="{{url('sponsor-a-meal')}}">Sponsor a Meal</a></li>
				@if(CustomHelper::isAllowedModule('blogs'))
				<li><a href="{{url('blogs')}}">Blogs</a></li>
				@endif
				<li class="highlight-link"><a href="{{url('contact-us')}}">Contact Us</a></li>
			</ul> 
		</div>
		<?php // } ?>
	</div>
